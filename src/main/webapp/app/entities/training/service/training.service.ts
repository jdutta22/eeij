...
export class TrainingService {
  ...
  protected convertDateFromClient(training: ITraining): ITraining {
    return Object.assign({}, training, {
      trainingStartDate: training.trainingStartDate?.isValid() ? training.trainingStartDate.format(DATE_FORMAT) : undefined,
      traingEndDate: training.traingEndDate?.isValid() ? training.traingEndDate.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.trainingStartDate = res.body.trainingStartDate ? dayjs(res.body.trainingStartDate) : undefined;
      res.body.traingEndDate = res.body.traingEndDate ? dayjs(res.body.traingEndDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((training: ITraining) => {
        training.trainingStartDate = training.trainingStartDate ? dayjs(training.trainingStartDate) : undefined;
        training.traingEndDate = training.traingEndDate ? dayjs(training.traingEndDate) : undefined;
      });
    }
    return res;
  }
